...
// Handle feedback update (status locked to “approved” here)
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['case_id'])) {
    $cid      = intval($_POST['case_id']);
    $feedback = mysqli_real_escape_string($conn, $_POST['feedback']);

    mysqli_query($conn, "
      UPDATE approved_cases
      SET feedback   = '$feedback',
          updated_at = NOW()
      WHERE case_id = $cid
    ");

    header("Location: view_approved_cases.php?" . $_SERVER['QUERY_STRING']);
    exit();
}

// Fetch only approved cases from the archive
$sql = "
  SELECT ac.*, u.username
  FROM approved_cases ac
  JOIN users u ON ac.filed_by_user_id = u.user_id
  $whereSQL
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .filter-bar input[type="text"] {
      flex: 1; padding: 0.6rem 1rem; border:1px solid #ccc; border-radius:4px; font-size:1rem; max-width:800px;
      transition: border-color .2s ease;
    }
    .filter-bar input:focus { border-color:#1ABC9C; outline:none; }
    .filter-bar button {
      padding:0.6rem 1.2rem; background:#2B295F; color:#fff; border:none; border-radius:4px; cursor:pointer;
      transition: background .2s ease;
    }
    .filter-bar button:hover { background:#16A085; }
    .filter-bar a {
      display:inline-block; padding:0.6rem 1.2rem; background:#e74c3c; color:#fff; text-decoration:none; border-radius:4px;
      transition: background .2s ease;
    }
    ...
    .cases-table {
      ...
      box-shadow:0 1px 4px rgba(0,0,0,0.1); border:1px solid #bbb;
      border-radius:4px; overflow:hidden; margin-top:16px;
    }
    .cases-table th, .cases-table td {
      padding:6px 8px; text-align:left; border-bottom:1px solid #bbb; border-right:1px solid #bbb;
      font-size:0.9em; vertical-align:middle; background-clip:padding-box;
    }
    .cases-table th {
      background:#2F4F4F; color:#fff; font-weight:600; text-transform:uppercase;
    }
    ...
    /* Feedback-only action cell */
    .action-links textarea, .action-links button {
      width:100%; margin:4px 0; font-size:0.8em; padding:4px 6px; border-radius:3px; border:1px solid #ccc;
    }
    .action-links button {
...
<body>
  ...
  <main class="main-content">
    ...
    <form method="GET" class="filter-bar">
      ...
      <a href="view_approved_cases.php">Reset</a>
    </form>
